...
Arange=[01.0 05.0 10.0 20.0 30.0 50.0 99.0];
Nmax=2.0*round(2*sigma*max(dvect)+0.5);
v1h = zeros(length(Arange),Nmax);
v2h = zeros(length(Arange),Nmax);


for elem = 1:length(Arange)
    htry=0.5;
...
title('Convergence of first 4\sigma d_1 terms of V_{1+}(\alpha_{n,1}) vs. M')
xlabel('n','FontSize',14)
ylabel('|V_{1+}(\alpha_{n,1})|','FontSize',14)
for n=1:M
    temp=sprintf('%02d', Arange(n));
    entry(n,:)=strcat('M=',temp,' ');
end
h=legend(entry);
set(h,'FontSize',12);
...
title('Convergence of first 4\sigma d_1 terms of V_{2+}(\alpha_{n,1}) vs. M')
xlabel('n','FontSize',14)
